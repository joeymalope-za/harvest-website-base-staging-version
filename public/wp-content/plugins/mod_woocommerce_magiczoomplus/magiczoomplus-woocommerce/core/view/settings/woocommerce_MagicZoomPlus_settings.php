<?php
    ...
    if (!function_exists('magictoolbox_WooCommerce_MagicZoomPlus_get_wordpress_image_sizes')) {
        function magictoolbox_WooCommerce_MagicZoomPlus_get_wordpress_image_sizes( $unset_disabled = true ) {
    ...
    function WooCommerceMagicZoomPlus_get_description(&$description) {
        $result = '';
    ...
    function WooCommerceMagicZoomPlus_widthout_img($id) {
        $result = false;
    ...
    function WooCommerceMagicZoomPlus_get_options_groups ($settings, $profile = 'default', $map, $id, $corePath) {
        $html = '';
        $toolAbr = '';
        ...
        $imgSizes = magictoolbox_WooCommerce_MagicZoomPlus_get_wordpress_image_sizes();
            ...
            switch($s["type"]) {
                case "array":
                    ...
                    foreach($s["values"] as $p) {
                        ...
                        $pName = ucwords($p);
                        if(strtolower($p) == "yes") {
                            if (WooCommerceMagicZoomPlus_widthout_img(strtolower($s['id']))) {
                                $rButtons[strtolower($p)] .= WooCommerceMagicZoomPlus_get_description($description2);
                            } else {
                    ...
                    break;
                case "num":
                    $html .= '<input  style="width:60px;" type="text" name="magiczoomplussettings'.'_'.ucwords($profile).'_'.$name.'" id="magiczoomplussettings'.'_'.ucwords($profile).'_'. $name.'" value="'.$s["value"].'" />';
                    break;
                case "text":
                    ...
                    break;
                case "dropdown":
                    $html .= '<select name="magiczoomplussettings'.'_'.ucwords($profile).'_'.$name.'" id=magiczoomplussettings'.'_'.ucwords($profile).'_'. $name.'">';
...
<h1>Magic Zoom Plus settings</h1>
<br/>
<p style="margin-right:20px; float:right; font-size:15px; white-space: nowrap;">
        &nbsp;<a href="<?php echo WooCommerceMagicZoomPlus_url('http://www.magictoolbox.com/magiczoomplus/modules/woocommerce/',' configuration page resources settings link'); ?>" target="_blank">Documentation<span class="dashicons dashicons-share-alt2" style="text-decoration: none;line-height:1.3;margin-left:5px;"></span></a>&nbsp;|
        &nbsp;<a href="<?php echo WooCommerceMagicZoomPlus_url('http://www.magictoolbox.com/magiczoomplus/examples/',' configuration page resources examples link'); ?>" target="_blank">Examples<span class="dashicons dashicons-share-alt2" style="text-decoration: none;line-height:1.3;margin-left:5px;"></span></a>&nbsp;|
        &nbsp;<a href="<?php echo WooCommerceMagicZoomPlus_url('http://www.magictoolbox.com/contact/','configuration page resources support link'); ?>" target="_blank">Support<span class="dashicons dashicons-share-alt2" style="text-decoration: none;line-height:1.3;margin-left:5px;"></span></a>&nbsp;
        |&nbsp;<a href="<?php echo WooCommerceMagicZoomPlus_url('http://www.magictoolbox.com/buy/magiczoomplus/','configuration page resources buy link'); ?>" target="_blank">Buy<span class="dashicons dashicons-share-alt2" style="text-decoration: none;line-height:1.3;margin-left:5px;"></span></a>
</p>
    ...
    <div id="tabs">
        <h2 class="nav-tab-wrapper">
            <ul>
                ...
                <?php foreach (WooCommerceMagicZoomPlus_getParamsProfiles() as $block_id => $block_name) {
                    if (!isset($tactive)) {
        ...
        <div id="tab-default">
        <?php
            //echo WooCommerceMagicZoomPlus_get_options_groups($settings, 'default', array('default' => array('Watermark' => $map['default']['Watermark'])),$id);
            echo WooCommerceMagicZoomPlus_get_options_groups($settings, 'default', array('default' => array('Watermark' => $map['default']['Watermark'],
        ...
        <?php 
            foreach (WooCommerceMagicZoomPlus_getParamsProfiles() as $block_id => $block_name) {
            if ($block_id == 'default') continue;
            ?>
            <div id="tab-<?php echo $block_id; ?>">
                <?php echo WooCommerceMagicZoomPlus_get_options_groups($settings, $block_id, $map, $id, $corePath); ?>
            </div>
        <?php }  ?>
    </div>

    <p id="set-main-settings"><input type="submit" name="submit" class="button-primary" value="Save settings" />&nbsp;<a id="resetLink" style="color:red; margin-left:25px;" href="admin.php?page=WooCommerceMagicZoomPlus-config-page&reset_settings=true">Reset to defaults</a></p>
</form>

<!-- === onlyForMod start: wordpress -->
<div style="font-size:12px;margin:5px auto;text-align:center;">Learn more about the <a href="http://www.magictoolbox.com/magiczoomplus_integration/" target="_blank">customisation options<span class="dashicons dashicons-share-alt2" style="text-decoration: none;margin-left:2px;"></span></a></div>
<!-- === onlyForMod end -->