...
function WooCommerce_MagicZoomPlus_activate () {

    set_transient( 'WooCommerce_MagicZoomPlus_welcome_license_activation_redirect', true, 30 );
    ...
    magictoolbox_WooCommerce_MagicZoomPlus_create_db();

    magictoolbox_WooCommerce_MagicZoomPlus_init();

    WooCommerce_MagicZoomPlus_send_stat('install');
}

function WooCommerce_MagicZoomPlus_deactivate () {}

function WooCommerce_MagicZoomPlus_uninstall() {

    magictoolbox_WooCommerce_MagicZoomPlus_delete_row_from_db('WooCommerce_MagicZoomPlus_magicscroll');
    magictoolbox_WooCommerce_MagicZoomPlus_delete_row_from_db();

    if (magictoolbox_WooCommerce_MagicZoomPlus_is_empty_db() && !count(WooCommerce_MagicZoomPlus_get_active_modules())) {
        magictoolbox_WooCommerce_MagicZoomPlus_remove_db();
    }

    delete_option("WooCommerceMagicZoomPlusCoreSettings");
    WooCommerce_MagicZoomPlus_send_stat('uninstall');
}

function WooCommerce_MagicZoomPlus_get_active_modules() {
...
function WooCommerce_MagicZoomPlus_send_stat($action = '') {

    //NOTE: don't send from working copy
    if('working' == 'v6.9.3' || 'working' == 'v5.3.7') {
        return;
    }
    ...
    $path = "api/stat/?action={$action}&tool_name=magiczoomplus&license=trial&tool_version=v5.3.7&module_version=v6.9.3&platform_name=woocommerce&platform_version={$platformVersion}&url={$url}";
...
add_filter( 'woocommerce_product_data_tabs', 'WooCommerce_MagicZoomPlus_add_videos_product_data_tab' , 99 , 1 );
function WooCommerce_MagicZoomPlus_add_videos_product_data_tab( $product_data_tabs ) {
...
add_action( 'woocommerce_product_data_panels', 'WooCommerce_MagicZoomPlus_add_videos_product_data_fields' );
function WooCommerce_MagicZoomPlus_add_videos_product_data_fields() {
...
add_action( 'woocommerce_process_product_meta', 'WooCommerce_MagicZoomPlus_woocommerce_process_product_meta_fields_save' );
function WooCommerce_MagicZoomPlus_woocommerce_process_product_meta_fields_save( $post_id ){
    ...
    if (isset( $_POST['_provide_videolinks_field'])){
        ...
        foreach($urls as $key => $_url) {
            ...
            if($isVimeo) {
                ...
                $thumb = 'https://i1.ytimg.com/vi/'.$videoCode.'/0.jpg';
            }
...
function showMessage_WooCommerce_MagicZoomPlus($message, $errormsg = false) {
    if ($errormsg) {
...
function showAdminMessages_WooCommerce_MagicZoomPlus(){
    global $error_message;
    if (current_user_can('edit_posts')) {
       showMessage_WooCommerce_MagicZoomPlus($error_message,true);
    }
}

function WooCommerce_MagicZoomPlus_LoadScroll($tool) {
...
function plugin_get_version_WooCommerce_MagicZoomPlus() {
    $plugin_data = get_plugin_data(dirname(plugin_dir_path(__FILE__)).'/mod_woocommerce_magiczoomplus.php');
    $plugin_version = $plugin_data['Version'];
    return $plugin_version;
}

function update_plugin_message_WooCommerce_MagicZoomPlus() {
    ...
    if (version_compare($oldVer, $ver, '<')) {
        echo '<div id="message" class="updated fade">
                  <p>New version available! We recommend that you download the <a href="'.WooCommerceMagicZoomPlus_url('http://magictoolbox.com/magiczoomplus/modules/woocommerce/',' plugins page update link ').'">latest version</a> of Magic Zoom Plus for WooCommerce . </p>
              </div>';
    }
}

function get_tool_version_WooCommerce_MagicZoomPlus($tool=null) {
...
function  magictoolbox_WooCommerce_MagicZoomPlus_init() {

    add_action( 'admin_init', 'WooCommerceMagicZoomPlus_welcome_license_do_redirect' );

    global $error_message;


    add_action("admin_menu", "magictoolbox_WooCommerce_MagicZoomPlus_config_page_menu");
    add_action('admin_enqueue_scripts', 'WooCommerce_MagicZoomPlus_load_admin_scripts');
    add_action('wp_enqueue_scripts', 'WooCommerce_MagicZoomPlus_load_frontend_scripts');
    ...
    add_action('wp_ajax_WooCommerce_MagicZoomPlus_import', 'WooCommerce_MagicZoomPlus_import');
    add_action('wp_ajax_WooCommerce_MagicZoomPlus_export', 'WooCommerce_MagicZoomPlus_export');


    add_action('wp_ajax_magictoolbox_WooCommerce_MagicZoomPlus_set_license', 'magictoolbox_WooCommerce_MagicZoomPlus_set_license');


    add_filter( 'woocommerce_ajax_variation_threshold', 'magictoolbox_WooCommerce_MagicZoomPlus_magictoolbox_wc_ajax_variation_threshold', 10, 2 );

    
    
    
    add_filter( 'plugin_action_links', 'magictoolbox_WooCommerce_MagicZoomPlus_links', 10, 2 );
    add_filter( 'plugin_row_meta', 'magictoolbox_WooCommerce_MagicZoomPlus_plugin_row_meta' , 10, 2 );
    ...
    if ($error_message) add_action('admin_notices', 'showAdminMessages_WooCommerce_MagicZoomPlus');

    if(!isset($GLOBALS['magictoolbox']['WooCommerceMagicZoomPlus'])) {
        require_once(dirname(__FILE__) . '/core/magiczoomplus.module.core.class.php');
        $coreClassName = "MagicZoomPlusModuleCoreClass";
        $GLOBALS['magictoolbox']['WooCommerceMagicZoomPlus'] = new $coreClassName;
        $coreClass = &$GLOBALS['magictoolbox']['WooCommerceMagicZoomPlus'];
    }
    $coreClass = &$GLOBALS['magictoolbox']['WooCommerceMagicZoomPlus'];
    /* get current settings from db */
    $settings = get_option("WooCommerceMagicZoomPlusCoreSettings");
    if($settings !== false && is_array($settings) && isset($settings['default']) && !isset($_GET['reset_settings'])) {
        foreach (WooCommerceMagicZoomPlus_getParamsProfiles() as $profile => $name) {
        ...
        $map = WooCommerceMagicZoomPlus_getParamsMap();
    ...
    delete_option("WooCommerceMagicZoomPlusCoreSettings");
        add_option("WooCommerceMagicZoomPlusCoreSettings", $allParams,'','no');
    }

    add_action( 'upgrader_process_complete', 'WooCommerce_MagicZoomPlus_get_packed_js', 10, 2 );
    
    if (!$coreClass->params->checkValue('page-status','No','product')) {
        add_filter( 'woocommerce_locate_template', 'WooCommerce_MagicZoomPlus_locate_template', 100, 3 );
    }
    if (!$coreClass->params->checkValue('page-status','No','category')) {
        add_action( 'woocommerce_before_shop_loop_item', 'magictoolbox_WooCommerce_MagicZoomPlus_start_parsing', 10, 3 );
        add_action( 'woocommerce_after_shop_loop_item', 'magictoolbox_WooCommerce_MagicZoomPlus_end_parsing', 10, 3 );
    }
}

function WooCommerce_MagicZoomPlus_init_wp_filesystem($form_url) {
    global $wp_filesystem;
...
function WooCommerce_MagicZoomPlus_write_file ($url, $content) {
    global $wp_filesystem;
    // if (empty($wp_filesystem)) {
    //     require_once (ABSPATH . '/wp-admin/includes/file.php');
    // }
    WooCommerce_MagicZoomPlus_init_wp_filesystem($url);
...
function WooCommerce_MagicZoomPlus_rewrite ($option, $tool) {
    $response = get_option($option);
    $result = WooCommerce_MagicZoomPlus_write_file(plugin_dir_path(__FILE__).'core/'.$tool.'.js', $response);
    //delete_option($option);
    return $result;
}

function WooCommerce_MagicZoomPlus_get_packed_js ($upgrader_object, $options) {
    if ('update' == $options['action'] && 'plugin' == $options['type']) {
        foreach ($options['plugins'] as $pl) {
            $_plugin = explode("/", $pl);
            $_plugin = $_plugin[count($_plugin) - 1];
            if ('mod_woocommerce_magiczoomplus.php' === $_plugin) {
                $key = magictoolbox_WooCommerce_MagicZoomPlus_get_data_from_db();
                if (!$key) {
                    $result = WooCommerce_MagicZoomPlus_rewrite("WooCommerce_MagicZoomPlus_backup", 'magiczoomplus');
                }
                $result2 = WooCommerce_MagicZoomPlus_rewrite("WooCommerce_MagicZoomPlus_magicscroll_backup", 'magicscroll');
                break;
            }
        }
    }
...
function WooCommerce_MagicZoomPlus_load_frontend_scripts () {
    ...
    add_action("wp_footer", "magictoolbox_WooCommerce_MagicZoomPlus_add_options_script", 10001);
}

function WooCommerce_MagicZoomPlus_load_admin_scripts () {
    ...
    if (array_key_exists('page', $_GET)) {
        $ownPage =  "WooCommerceMagicZoomPlus-config-page" ==  $_GET["page"]        ||
                    "WooCommerceMagicZoomPlus-shortcodes-page" ==  $_GET["page"]    ||
                    "WooCommerceMagicZoomPlus-import-export-page" ==  $_GET["page"] ||
                    "WooCommerceMagicZoomPlus-license-page" ==  $_GET["page"];
    }

    if (is_admin()) {
        wp_register_script( 'woocommerce_MagicZoomPlus_admin_adminpage_script', plugin_dir_url( __FILE__ ).'core/woocommerce_MagicZoomPlus_adminpage.js', array('jquery', 'jquery-ui-core', 'jquery-ui-tabs'), null );
        wp_enqueue_style( 'magictoolbox_woocommerce_MagicZoomPlus_admin_menu_style', plugin_dir_url( __FILE__ ).'core/admin_menu.css', array(), null );
        if ($ownPage) {
            wp_enqueue_style( 'magictoolbox_woocommerce_MagicZoomPlus_admin_page_style', plugin_dir_url( __FILE__ ).'core/admin.css', array(), null );
        }

        if ($ownPage) {
            wp_enqueue_style( 'WooCommerce_MagicZoomPlus_admin_import_export_style', plugin_dir_url( __FILE__ ).'core/view/import_export/woocommerce_MagicZoomPlus_import_export.css', array(), null );
            wp_enqueue_style( 'WooCommerce_MagicZoomPlus_admin_license_style', plugin_dir_url( __FILE__ ).'core/view/license/woocommerce_MagicZoomPlus_license.css', array(), null );
        }
        wp_register_script( 'WooCommerce_MagicZoomPlus_admin_import_export_script', plugin_dir_url( __FILE__ ).'core/view/import_export/woocommerce_MagicZoomPlus_import_export.js', array('jquery'), null );
        wp_register_script( 'WooCommerce_MagicZoomPlus_admin_license_script', plugin_dir_url( __FILE__ ).'core/view/license/woocommerce_MagicZoomPlus_license.js', array('jquery'), null );
    }
}
function magictoolbox_WooCommerce_MagicZoomPlus_magictoolbox_wc_ajax_variation_threshold( $qty, $product ) {
    return 150;
}

/*
function magictoolbox_WooCommerce_MagicZoomPlus_denided_prettyPhoto_inline(){
...
function magictoolbox_WooCommerce_MagicZoomPlus_plugin_row_meta( $links, $file ) {

    if (strpos(plugin_dir_path(__FILE__),plugin_dir_path($file))) {
        $row_meta = array($links[0],$links[1]);
        $row_meta['Settings'] = '<a href="admin.php?page=WooCommerceMagicZoomPlus-config-page">'.__('Settings').'</a>';
        $row_meta['Support'] =  '<a target="_blank" href="'.WooCommerceMagicZoomPlus_url('https://www.magictoolbox.com/contact/','plugins page support link').'">Support</a>';
        $row_meta['Buy'] = '<a target="_blank" href="'.WooCommerceMagicZoomPlus_url('https://www.magictoolbox.com/buy/magiczoomplus/','plugins page buy link').'">Buy</a>';
        $row_meta['More cool plugins'] = '<a target="_blank" href="'.WooCommerceMagicZoomPlus_url('https://www.magictoolbox.com/woocommerce/','plugins page more cool plugins link').'">More cool plugins</a>';
...
function WooCommerceMagicZoomPlus_config_page() {
    include 'core/view/settings/woocommerce_MagicZoomPlus_settings.php';
}

function WooCommerce_MagicZoomPlus_add_admin_src_to_menu_page() {
    wp_enqueue_script( 'woocommerce_MagicZoomPlus_admin_adminpage_script' );
    ...
    wp_localize_script( 'woocommerce_MagicZoomPlus_admin_adminpage_script', 'magictoolbox_WooCommerce_MagicZoomPlus_admin_modal_object', $arr);
}

function WooCommerce_MagicZoomPlus_import() {
    if(!(is_array($_POST) && defined('DOING_AJAX') && DOING_AJAX)){
        return;
    }

    $nonce = $_POST['nonce'];
    $tool = 'woocommerce_magiczoomplus';
    ...
    if (array_key_exists('tool', $arr) && $tool == $arr['tool']) {
        if (array_key_exists('license', $arr) && $arr['license'] != 'trial' && strlen($arr['license']) == 7) {
            magictoolbox_WooCommerce_MagicZoomPlus_update_db($arr['license']);

            $url = 'https://www.magictoolbox.com/site/order/'.$arr['license'].'/magiczoomplus.js';
            $response = magictoolbox_WooCommerce_MagicZoomPlus_get_file($url);
            if($response['status'] == 200) {
                WooCommerce_MagicZoomPlus_write_file(plugin_dir_path( __FILE__ ).'core/magiczoomplus.js', $response['content']);
            }
        }
        if (array_key_exists('scrolllicense', $arr) && $arr['scrolllicense'] != 'trial' && strlen($arr['scrolllicense']) == 7) {
            magictoolbox_WooCommerce_MagicZoomPlus_update_db($arr['scrolllicense'], 'WooCommerce_MagicZoomPlus_magicscroll');
            $url = 'https://www.magictoolbox.com/site/order/'.$arr['scrolllicense'].'/magicscroll.js';
            $response = magictoolbox_WooCommerce_MagicZoomPlus_get_file($url);
            if($response['status'] == 200) {
                WooCommerce_MagicZoomPlus_write_file(plugin_dir_path( __FILE__ ).'core/magicscroll.js', $response['content']);
            }
        }
        ...
        if (array_key_exists('core', $arr)) {
            ...
            delete_option("WooCommerceMagicZoomPlusCoreSettings");
            add_option("WooCommerceMagicZoomPlusCoreSettings", $settings,'','no');
        }
...
function WooCommerce_MagicZoomPlus_export() {
    if(!(is_array($_POST) && defined('DOING_AJAX') && DOING_AJAX)){
        return;
    }
    ...
    //WooCommerce_MagicZoomPlus_wp_export($value, get_option("WooCommerceMagicZoomPlusCoreSettings"), $secret_data);
    WooCommerce_MagicZoomPlus_wp_export(get_option("WooCommerceMagicZoomPlusCoreSettings"), $secret_data, $value);
    exit;
}

function magictoolbox_WooCommerce_MagicZoomPlus_add_src_to_footer() {
    ...
    if (!$magictoolbox_MagicZoomPlus_page_added_script) {
        ...
        if ($magictoolbox_MagicZoomPlus_page_has_tool) {
            ...
            // wp_enqueue_style('magictoolbox_magiczoomplus_module_style');
            // wp_enqueue_script('magictoolbox_magiczoomplus_script');
            if (!WooCommerce_MagicZoomPlus_page_check('WooCommerce')) {
                wp_enqueue_script( 'magictoolbox-product', plugins_url('/core/woocommerce_MagicZoomPlus_product.js', __FILE__));
            }
        }
    }
...
function magictoolbox_WooCommerce_MagicZoomPlus_add_options_script () {
    global $magictoolbox_MagicZoomPlus_page_added_options,
            $magictoolbox_MagicZoomPlus_page_has_shortcode,
            $magictoolbox_MagicZoomPlus_page_has_tool;
    $footers = '';
    $cat = WooCommerce_MagicZoomPlus_page_check('WooCommerce');
    ...
    if (!$magictoolbox_MagicZoomPlus_page_added_options) {
        ...
        if ($magictoolbox_MagicZoomPlus_page_has_shortcode || $magictoolbox_MagicZoomPlus_page_has_tool) {
            ...
            if (!$cat) {
                $jsonVariations = WooCommerce_MagicZoomPlus_get_product_variations();
...
function magictoolbox_WooCommerce_MagicZoomPlus_get_file($url) {
...
function magictoolbox_WooCommerce_MagicZoomPlus_set_license() {
    global $wp_filesystem;
    ...
    if (!$extra_param || 'null' == $extra_param) {
        ...
        $extra_param = 'WooCommerce_MagicZoomPlus_'.$extra_param;
    }
    ...
    if ( !wp_verify_nonce( $nonce, 'magic-everywhere' ) ) {
        ...
        if ($key && '' != $key) {
            ...
            if($code == 200) {
                $result = WooCommerce_MagicZoomPlus_write_file(plugin_dir_path( __FILE__ ).'core/'.$tool_name.'.js', $response);
                if (!$result) {
                    magictoolbox_WooCommerce_MagicZoomPlus_update_db($key, $extra_param);
                    $result = 'null';
                }
                $result = '{"error": '.$result.'}';
            } else if($code == 403) {
...
function magictoolbox_WooCommerce_MagicZoomPlus_create_db() {
    global $wpdb;
...
function magictoolbox_WooCommerce_MagicZoomPlus_remove_db() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'magictoolbox_store';
...
function magictoolbox_WooCommerce_MagicZoomPlus_update_db($key, $name=null) {
    global $wpdb;
    $result = false;

    if (!$name || !is_string($name)) {
        $name = 'WooCommerce_MagicZoomPlus';
    }
...
function magictoolbox_WooCommerce_MagicZoomPlus_delete_row_from_db($name=null) {
    global $wpdb;
...
function magictoolbox_WooCommerce_MagicZoomPlus_is_empty_db() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'magictoolbox_store';
    if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") == $table_name) {
...
function magictoolbox_WooCommerce_MagicZoomPlus_get_data_from_db($name=null) {
    global $wpdb;
    ...
    if (isset($GLOBALS['WooCommerce_MagicZoomPlus_get_data_from_db'][$name])) {
        return $GLOBALS['WooCommerce_MagicZoomPlus_get_data_from_db'][$name];
    }
    ...
    if ($result && count($result) > 0) {
        $GLOBALS['WooCommerce_MagicZoomPlus_get_data_from_db'][$name] = $result[0];
        return $result[0];
    } else {
        $GLOBALS['WooCommerce_MagicZoomPlus_get_data_from_db'][$name] = false;
        return false;
    }
}

function magictoolbox_WooCommerce_MagicZoomPlus_links( $links, $file ) {
    $fileName = 'mod_woocommerce_magiczoomplus_trial/mod_woocommerce_magiczoomplus.php';
    
    $fileName = preg_replace('/\_trial\//', '/', $fileName);
    $fileName = preg_replace('/\_commercial\//', '/', $fileName);
    
    if ($file == $fileName) {
        $settings_link = '<a href="admin.php?page=WooCommerceMagicZoomPlus-config-page">'.__('Settings').'</a>';
        array_unshift( $links, $settings_link );
    }
    return $links;
}

function magictoolbox_WooCommerce_MagicZoomPlus_config_page_menu() {
    if(function_exists("add_menu_page")) {
        $page = add_menu_page(__("Magic Zoom Plus for WooCommerce"), __("Magic Zoom Plus for WooCommerce"), "edit_posts", "WooCommerceMagicZoomPlus-config-page", "WooCommerceMagicZoomPlus_config_page", plugin_dir_url( __FILE__ )."core/admin_graphics/icon.svg");
        add_submenu_page( "WooCommerceMagicZoomPlus-config-page", 'Settings', 'Settings', 'edit_posts', "WooCommerceMagicZoomPlus-config-page" );
        add_action('admin_print_scripts-' . $page, 'WooCommerce_MagicZoomPlus_add_admin_src_to_menu_page');
    }

    if(function_exists("add_submenu_page")) {

        $license_page = add_submenu_page("WooCommerceMagicZoomPlus-config-page", "License", "License", "edit_posts", "WooCommerceMagicZoomPlus-license-page", "WooCommerce_MagicZoomPlus_license_page");
        add_action('admin_print_scripts-' . $license_page, 'WooCommerce_MagicZoomPlus_add_admin_src_to_license_page');
        $import_export_page = add_submenu_page("WooCommerceMagicZoomPlus-config-page", "Backup / Restore", "Backup / Restore", "edit_posts", "WooCommerceMagicZoomPlus-import-export-page", "WooCommerce_MagicZoomPlus_import_export_page");
        add_action('admin_print_scripts-' . $import_export_page, 'WooCommerce_MagicZoomPlus_add_admin_src_to_import_export_page');

    }
}


function WooCommerce_MagicZoomPlus_import_export_page() {
    include 'core/view/import_export/woocommerce_MagicZoomPlus_import_export.php';
}

function WooCommerce_MagicZoomPlus_add_admin_src_to_import_export_page() {
    wp_enqueue_script( 'WooCommerce_MagicZoomPlus_admin_import_export_script' );
    wp_localize_script( 'WooCommerce_MagicZoomPlus_admin_import_export_script', 'magictoolbox_WooCommerce_MagicZoomPlus_admin_modal_object', array('ajax' =>  get_site_url().'/wp-admin/admin-ajax.php', 'nonce' => wp_create_nonce('magic-everywhere')) );
}

function WooCommerce_MagicZoomPlus_license_page() {
    include 'core/view/license/woocommerce_MagicZoomPlus_license.php';
}

function WooCommerce_MagicZoomPlus_add_admin_src_to_license_page() {
    wp_enqueue_script( 'WooCommerce_MagicZoomPlus_admin_license_script' );
    wp_localize_script( 'WooCommerce_MagicZoomPlus_admin_license_script', 'magictoolbox_WooCommerce_MagicZoomPlus_admin_modal_object', array('ajax' =>  get_site_url().'/wp-admin/admin-ajax.php', 'nonce' => wp_create_nonce('magic-everywhere')) );
}

function magictoolbox_WooCommerce_MagicZoomPlus_styles() {
    if(!defined('MAGICTOOLBOX_MAGICZOOMPLUS_HEADERS_LOADED')) {
        ...
        $headers = $plugin->getHeadersTemplate($core_url."/{$path}/core");
            ...
            //$headers .= '<script type="text/javascript" src="'. $core_url .'/'. $path .'/core/woocommerce_MagicZoomPlus_product.js"></script>';
        echo $headers;
        define('MAGICTOOLBOX_MAGICZOOMPLUS_HEADERS_LOADED', true);
    }
}

function magictoolbox_WooCommerce_MagicZoomPlus_start_parsing () {
    $GLOBALS['magictoolbox']['WooCommerce_MagicZoomPlus']['parse_status'] = 'started';
    ob_start();
}

function magictoolbox_WooCommerce_MagicZoomPlus_end_parsing () {
    if (!isset($GLOBALS['magictoolbox']['WooCommerce_MagicZoomPlus']['parse_status'])) return;
    $GLOBALS['magictoolbox']['WooCommerce_MagicZoomPlus']['parse_status'] = 'ended';
    $content = ob_get_contents();
    ob_end_clean();
    $content = magictoolbox_WooCommerce_MagicZoomPlus_create($content);
    echo $content;
}

function magictoolbox_WooCommerce_MagicZoomPlus_start_alternative_parsing () {
    if (!isset($GLOBALS['magictoolbox']['WooCommerce_MagicZoomPlus']['parse_status'])) ob_start();
}

function magictoolbox_WooCommerce_MagicZoomPlus_end_alternative_parsing () {
    if (!isset($GLOBALS['magictoolbox']['WooCommerce_MagicZoomPlus']['parse_status'])) {
        $content = ob_get_contents();
        ob_end_clean();
        $content = magictoolbox_WooCommerce_MagicZoomPlus_create($content);
        echo $content;
...
function magictoolbox_WooCommerce_MagicZoomPlus_contentClean ($content) {
    global $wp_query;
    $plugin = $GLOBALS['magictoolbox']['WooCommerceMagicZoomPlus'];
...
function  magictoolbox_WooCommerce_MagicZoomPlus_create($content) {
    global $magictoolbox_MagicZoomPlus_page_has_tool;
    /*add_action( 'wp_print_footer_scripts', 'magictoolbox_WooCommerce_MagicZoomPlus_denided_prettyPhoto_inline' );*/
    ...
    $post_id = $wp_query->post->ID;
        ...
        $content = preg_replace_callback("/{$pattern}/is","magictoolbox_WooCommerce_MagicZoomPlus_callback",$content);
    ...
    if (!$plugin->params->checkValue('template','original') && $plugin->type == 'standard' && isset($GLOBALS['magictoolbox']['MagicZoomPlus']['main'])) {
        ...
        if (!WooCommerce_MagicZoomPlus_page_check('WooCommerce')) { //do not render thumbs on category pages
            $thumbs = WooCommerce_MagicZoomPlus_get_prepared_selectors($post_id);
        } else {
            $thumbs = array();
        }
    ...
    if (!isset($GLOBALS['magictoolbox']['prods_info']['product_id']) && isset($post_id)) {
        ...
        /*if (!$cat) {
            $html .= magictoolbox_WooCommerce_MagicZoomPlus_getMagicToolBoxEvent($plugin, $post_id);
        }*/
        ...
        if (isset($GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR'])) {
            $content = str_replace('{MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR}', $GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR'], $content);  //add main image selector to other
        }
    ...
    $content = str_replace('{MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR}','',$content);
    $content = str_replace('{MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_SELECTORS}','',$content); //if no selectors - remove constant
...
function magictoolbox_WooCommerce_MagicZoomPlus_key_sort($a, $b){
    return strnatcasecmp(basename($a['img']),basename($b['img']));
}

function magictoolbox_WooCommerce_MagicZoomPlus_check_plugin_active($plugin_id){
...
function magictoolbox_WooCommerce_MagicZoomPlus_getMagicToolBoxEvent($plugin, $post_id){
    ...
    $magictoolboxEvent = "
        ...
        <script type=\"text/javascript\">
            ...
            magicToolboxSwitchMetod = '".$mouseEvent."';
...
function  magictoolbox_WooCommerce_MagicZoomPlus_callback($matches) {
    $plugin = $GLOBALS['magictoolbox']['WooCommerceMagicZoomPlus'];
    $title = "";
    $float = "";
    $cat = WooCommerce_MagicZoomPlus_page_check('WooCommerce');
    ...
    WooCommerce_MagicZoomPlus_get_product_variations(); //call only for onload variation check
        ...
        if (!$cat) {
            ...
            $GLOBALS['magictoolbox']['MagicZoomPlus']['main'] = $plugin->getMainTemplate(compact('img','thumb','thumb2x','id','title','description','additionalDescription','link'));
        ...
        if (!$plugin->params->checkValue('create-main-image-selector','No') && !isset($GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SET'])) {
            $medium = WooCommerce_MagicZoomPlus_get_product_image($img_name,'thumb');
            $medium2x = WooCommerce_MagicZoomPlus_get_product_image($img_name,'thumb2x');
            $thumb = WooCommerce_MagicZoomPlus_get_product_image($img_name,'selector');
            $thumb2x = WooCommerce_MagicZoomPlus_get_product_image($img_name,'selector2x');
            //if (isset($title)) { $alt = $title; } else { $alt = ''; }
            $GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR'] = $GLOBALS['magictoolbox']['MagicZoomPlus']['selectors'][] = $plugin->getSelectorTemplate(compact('alt','img','medium','medium2x','thumb','thumb2x','id')); //save main image selector to globals
            $GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR'] = str_replace('<img','<img class="attachment-90x90" ',$GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR']);
        }
     ...
     if ($is_selector && !$is_main && !$cat) { //if image is SELECTOR
        ...
        $medium = WooCommerce_MagicZoomPlus_get_product_image($medium_name,'thumb');
        $medium2x = WooCommerce_MagicZoomPlus_get_product_image($medium_name,'thumb2x');
        $img = WooCommerce_MagicZoomPlus_get_product_image($medium_name,'original');
        $thumb = WooCommerce_MagicZoomPlus_get_product_image($medium_name,'selector');
        $thumb2x = WooCommerce_MagicZoomPlus_get_product_image($medium_name,'selector2x');
        if ($plugin->params->checkValue('template','original')) {
            ...
            $result = '';
            $GLOBALS['magictoolbox']['MagicZoomPlus']['selectors'][] = $plugin->getSelectorTemplate(compact('alt','img','medium','medium2x','thumb','thumb2x','id','title'));
        }

        if (!isset($GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR_SET'])) {
    ...
    if (empty($info)) $info = '';
     if ($is_main) {
        ...
        /*if (!$cat) {
            if (!isset($GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SET'])) $result = $matches[0];
            $GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SET'] = 'true';
        }*/ //TODO WHAT IS THIS ?
    } else if ($is_selector) {
...
function WooCommerce_MagicZoomPlus_get_product_image($title,$size = 'thumb', $image_id = false) {
    $cat = WooCommerce_MagicZoomPlus_page_check('WooCommerce');
    $result = $title;
...
function WooCommerce_MagicZoomPlus_get_post_attachments($addMain = false)  {
...
function WooCommerce_MagicZoomPlus_get_product_variations ($product_id = false) {
    ...
    $GLOBALS['MAGICTOOLBOX_'.strtoupper('MagicZoomPlus').'_VARIATIONS_SELECTORS'] = array();
    ...
    if ( $product->is_type('variable')) {
        ...
        if (is_array($variations) && count($variations) > 0) {
            ...
            foreach ($variations as $variation) {
                ...
                if (isset($variation['image_src']) && !empty($variation['image_src']) && isset($variation['image_link']) && !empty($variation['image_link'])) {
                    ...
                    if (!$useWpImages) {
                        ...
                        $img = WooCommerce_MagicZoomPlus_get_product_image($img_name,'original');
                        $thumb = WooCommerce_MagicZoomPlus_get_product_image($img_name);
                        $thumb2x = WooCommerce_MagicZoomPlus_get_product_image($img_name,'thumb2x');
                        $selector = WooCommerce_MagicZoomPlus_get_product_image($img_name,'selector');
                        $selector2x = WooCommerce_MagicZoomPlus_get_product_image($img_name,'selector2x');
            ...
            //$a = str_replace('<a','<a onclick="onMagicZoomPlusSelectorClick(this);" data-image="'.$variation['image_link'].'" ',$a);
            $a = str_replace('<a','<a data-product-id="'.$product->get_id().'" data-magic-slide-id="zoom" onclick="onMagicZoomPlusSelectorClick(this);" ',$a);

            $GLOBALS['MAGICTOOLBOX_'.strtoupper('MagicZoomPlus').'_VARIATIONS_SELECTORS'][] = $a;
            }
    ...
    if (empty($jsonVariations) || $jsonVariations === '' || $jsonVariations === false || $jsonVariations === 'false') {
        ...
        $jsonVariations = '<script type="text/javascript">
            ...
            (function($, isWoo301, prodId) {
                ...
                '.file_get_contents(dirname(__FILE__) . '/core/woocommerce_MagicZoomPlus_variations.js').'
...
function WooCommerce_MagicZoomPlus_get_prepared_selectors ($post_id, $useWpImages = false) {

    $attachments = WooCommerce_MagicZoomPlus_get_post_attachments($useWpImages);
...
function WooCommerce_MagicZoomPlus_page_check ($moduleName = false) {
...
function WooCommerceMagicZoomPlus_url ($url,$position) {

    if ('commercial' == get_tool_version_WooCommerce_MagicZoomPlus()) {
    $utm_source = 'CommercialVerison';
    } else {
    if (magictoolbox_WooCommerce_MagicZoomPlus_get_data_from_db()) {
...
// function WooCommerceMagicZoomPlus_params_map_check ($profile = 'default', $group, $parameter) {
//     $map = WooCommerceMagicZoomPlus_getParamsMap();
//     if (isset($map[$profile][$group][$parameter])) return true;
//     return false;
// }
function WooCommerceMagicZoomPlus_getParamsMap () {
    $map = array(
		'product' => array(
			'General' => array(
				'include-headers',
...
function WooCommerceMagicZoomPlus_getParamsProfiles () {
...
function WooCommerceMagicZoomPlus_welcome_license_do_redirect() {
  // Bail if no activation redirect
    if ( ! get_transient( 'WooCommerce_MagicZoomPlus_welcome_license_activation_redirect' ) ) {
    return;
  }

  // Delete the redirect transient
  delete_transient( 'WooCommerce_MagicZoomPlus_welcome_license_activation_redirect' );
  ...
  wp_safe_redirect( add_query_arg( array( 'page' => 'WooCommerceMagicZoomPlus-license-page' ), admin_url( 'admin.php' ) ) );

}

function WooCommerceMagicZoomPlus_plugin_path() {

  return untrailingslashit( plugin_dir_path( __FILE__ ) );
 
}
 
function WooCommerce_MagicZoomPlus_locate_template( $template, $template_name, $template_path ) {
  ...
  $plugin_path  = WooCommerceMagicZoomPlus_plugin_path() . '/core/templates/';
...
function WooCommerce_MagicZoomPlus_get_containers_data($thumbs = array(), $post_id = false, $useWpImages = false) {
    ...
    if ((metadata_exists( 'post', $post_id, '_magic360_data' ) || metadata_exists( 'post', $main_id, '_magic360_data' )) && magictoolbox_WooCommerce_MagicZoomPlus_check_plugin_active('magic360') ){
        ...
        if(!empty($magic360_plugin) && $magic360_plugin->params->checkValue('page-status','Yes') ){
            ...
            if(!empty($magic360_image_gallery)){
                ...
                if (!$useWpImages) {
                    $magic360_selector = '<a data-product-id="'.$post_id.'" data-magic-slide-id="360" style="display:inline-block;" class="m360-selector" title="360" href="#" onclick="return false;"><img src="'.WooCommerce_MagicZoomPlus_get_product_image('/'.ltrim($magic360_selector_path,'/'),'selector').'" alt="360" /></a>';
                } else {
                ...
                usort($GLOBALS['magic360images'], 'magictoolbox_WooCommerce_MagicZoomPlus_key_sort');
...
if( function_exists('register_block_type' ) ){
  if( !function_exists('WooCommerce_MagicZoomPlus_addmedia_block')){
    function WooCommerce_MagicZoomPlus_addmedia_block(){

      wp_register_script(
        'woocommerce-magiczoomplus-addmedia-block-editor-js',
        plugins_url('/gutenberg/addmedia-block/editor-script.js', __FILE__),
        array( 'wp-blocks', 'wp-element', 'wp-editor', 'jquery'), NULL
      );

      register_block_type( 'woocommerce-magiczoomplus/addmedia-block', array(
          'editor_script' => 'woocommerce-magiczoomplus-addmedia-block-editor-js',
      ) );
    }

    add_action( 'init', 'WooCommerce_MagicZoomPlus_addmedia_block' );
  }

}

function WooCommerce_MagicZoomPlus_slideshow_gallery($atts){

    global $wpdb;

    $table_name = $wpdb->prefix . strtolower('MagicZoomPlus_store');
    $result = $wpdb->get_results("SELECT id,name FROM $table_name ");

    return rest_ensure_response( $result );
}

add_action( 'rest_api_init', 'WooCommerce_MagicZoomPlus_gallery_route');

function WooCommerce_MagicZoomPlus_gallery_route() {
            
    register_rest_route( 'MagicZoomPlus', 'get-shortcodes', array(
            'methods' => 'GET',
            'callback' => 'WooCommerce_MagicZoomPlus_slideshow_gallery',
