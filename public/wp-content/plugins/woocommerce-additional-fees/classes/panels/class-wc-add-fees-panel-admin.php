...
class WC_Add_Fees_Panel_Admin
{
	...
	*/
	...
	public function &get_form_fields_settings()
	{
		...
		$this->fields[] = array(
				'type' => WC_Addons_Add_Fees::HIDDEN_INPUT,
				'id' => 'wc_add_fees_settings_last_tab_active',
	...
	public function get_gateway_inputfields( $key, $gateway )
	{
		 ...
		 * @param WC_Add_Fees_Panel_Admin $this
		 * @return array
		 */
		$this->fields = apply_filters( 'wc_add_fees_get_settings_options_gateway_default', $this->fields, $key, $gateway, $this );
		...
		$default = $this->options[WC_Add_Fees::OPT_GATEWAY_PREFIX][ $key ][WC_Add_Fees::OPT_KEY_FIXED_VALUE_POS];
		if( empty( $default ) )
		{
			$default = 'after';
		}
		$name = $this->create_unique_html_name( $key, WC_Add_Fees::OPT_KEY_FIXED_VALUE_POS );
		$this->fields[] = array(
			'type' => 'select',
			'id' => $name,
			'title' => __( 'Order to add fixed value:', WC_Add_Fees::TEXT_DOMAIN ),
			'default' => $default,
			'options' => WC_Add_Fees::$order_add_fixed_value,
			'desc' => __( 'Select the order when you want to add the fixed value. When adding before the fixed value is included in calculating the fee with value to add', WC_Add_Fees::TEXT_DOMAIN ),
			'desc_tip' => true
			);
		...
		$default = $this->options[WC_Add_Fees::OPT_GATEWAY_PREFIX][ $key ]['minvalue'];
		$name = $this->create_unique_html_name( $key, 'minvalue' );
		$this->fields[] = array(
			'type' => 'number',
			'custom_attributes' => array( 'step' => 'any', 'min' => 0 ),
			'id' => $name,
			'title' => __( 'Minimum cart value for adding fee:', WC_Add_Fees::TEXT_DOMAIN ),
			'default' => $default,
			'desc' => __( 'Ignore additional fee, when total amount is below this value. Leave 0 to ignore this setting. Enter the value according to your settings in \'Prices entered with/without tax\'.', WC_Add_Fees::TEXT_DOMAIN ),
			'desc_tip' => true
			);
		...
		$default = $this->options[WC_Add_Fees::OPT_GATEWAY_PREFIX][ $key ]['minimum_fee'];
		$name = $this->create_unique_html_name( $key, 'minimum_fee' );
		$this->fields[] = array(
			'type' => 'number',
			'custom_attributes' => array( 'step' => 'any', 'min' => 0 ),
			'id' => $name,
			'title' => __( 'Minimum fee:', WC_Add_Fees::TEXT_DOMAIN ),
			'default' => $default,
			'desc' => __( 'This is the minimum fee added, when the calculated fee is below this value. Leave 0 to ignore this setting. Enter the value according to your settings in \'Prices entered with/without tax\'.', WC_Add_Fees::TEXT_DOMAIN ),
			'desc_tip' => true
			);
	...
	public function save_options_settings()
	{
		...
		foreach( WC_Add_Fees::instance()->gateways as $key => &$gateway )
		{
			...
			$name = $this->create_unique_html_name( $key, WC_Add_Fees::OPT_KEY_FIXED_VALUE_POS );
			$value = isset( $_REQUEST[ $name ] ) ? stripslashes( $_REQUEST[ $name ] ) : 'after';
			$this->options[WC_Add_Fees::OPT_GATEWAY_PREFIX][ $key ][WC_Add_Fees::OPT_KEY_FIXED_VALUE_POS] = $value;
			...
			if( $this->check_numeric( $value, $name ) )
			{
				$this->options[WC_Add_Fees::OPT_GATEWAY_PREFIX][ $key ]['minvalue'] = $value;
			}
			...
			if( $this->check_numeric( $value, $name ) )
			{
				$this->options[WC_Add_Fees::OPT_GATEWAY_PREFIX][ $key ]['minimum_fee'] = $value;
			}
			 ...
			 * @return array
			 */
			$this->options = apply_filters( 'wc_add_fees_save_settings_options_gateway_default', $this->options, $key, $gateway, $this );
		}
		...
		$this->woo_addons->last_tab_active = isset( $_REQUEST[ 'wc_add_fees_settings_last_tab_active' ] ) ? $_REQUEST[ 'wc_add_fees_settings_last_tab_active' ] : '';
	 ...
	 * @param string $key
	 * @param string $id
	 * @param string $index			@added 3.1.8
	 * @return string
	 */
	public function create_unique_html_name( $key, $id, $index = '' )
	{
		return ( '' == $index ) ? $id . '___' . $key :  "{$id}___{$key}[{$index}]";
	}
}
