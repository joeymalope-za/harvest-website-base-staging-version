...
.vxcf_options_row .howto{
  ...
  <div id="crm_fields_map" class="vx_contents">
    <div class="vx_div ">
  <div class="vx_head ">
<div class="crm_head_div"> <?php esc_html_e('1. Select Zoho Account',  'woo-zoho' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vxc_account"><?php esc_html_e('Select Zoho Account.', 'woo-zoho');  $this->tooltip($tooltips['sel_object']); ?></label>
  </div>
  <div class="vx_col2">
  <div>
  ...
  <div id="crm_ajax_account" style="display:none; text-align: center; line-height: 100px;"><i><?php esc_html_e('Loading, Please Wait...', 'woo-zoho'); ?></i></div>
    <?php 
    $valid=true;
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','woo-zoho') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  button_state_vx("ajax",btn);
   var arr={action:'refresh_data_<?php echo esc_attr($this->id) ?>',vx_crm_ajax:vx_crm_ajax,vx_action:action,account:account,post_id:post_id};
  if(action == 'refresh_layouts'){
      arr['object']=$('#vxc_object').val();
  }
 ...
 if(re.data){
  ...
  if(re.status == "ok" && re.error && re.error!=""){
      alert(re.error);
  }   
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','woo-zoho') ?>');
     return;   
    }  
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  e.preventDefault(); 
  if(!confirm('<?php esc_html_e('Are you sure to remove ?','woo-zoho') ?>')){
      return;
  }
...
start_tooltip();
  ...
  if(object == ""){
  alert("<?php esc_html_e('Please Select Object','woo-zoho') ?>");
  return;
  }
  if(vx_ajax && vx_ajax.abort){
    vx_ajax.abort();  
...
$('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','woo-zoho') ?>'});
add_note_sel();
} 
  });
function add_note_sel(){
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','woo-zoho') ?>'});
}  
