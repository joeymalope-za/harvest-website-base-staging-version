...
ul.vx_notes li.customer-note .note_content:after {
  ...
  ?>
  <p>
  <button class="button" type="submit" name="<?php echo esc_attr($this->id) ?>_send" title="<?php esc_html_e("Send to Zoho",'woo-zoho')?>" value="yes"><?php esc_html_e("Send to Zoho",'woo-zoho')?></button>
  <?php
      if($comments ){
  $log_url=admin_url( 'admin.php?page='.$this->id.'_log&order_id='.$post_id); 
  ?>
  <a href="<?php echo esc_url($log_url) ?>" class="button" title="<?php esc_html_e("Go to Logs",'woo-zoho')?>"><?php esc_html_e("Go to Logs",'woo-zoho')?></a>
  <?php
      }
  ?>
  </p>