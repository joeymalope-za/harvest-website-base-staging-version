...
3. To allow working with media features (camera for id verification) from non-HTTPS sites in Chrome, add `http://harvest.local,http://id-api.local:3000,http://id-web.local:3002` to `chrome://flags/#unsafely-treat-insecure-origin-as-secure`
4. Install `git`
4. (Optional) install PHP 8 and Composer locally to debug the code with local interpreter
5. Clone this repo and `harvest-web` repo. You need to [add a GitHub SSH key](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent?platform=linux) first.
   ```
   git config core.fileMode false
   ```
5. Create `.env` file and fill in the variables from `docker-compose.yml`, which don't have defaults, except `WP_VERSION`:
   ```
   S3_KEY=...
   S3_SECRET=...
   ```
6. Run `scripts/start.sh`
7. Check `docker compose logs --tail 20` to see if all services have started without errors
8. The URLs:
   - http://harvest.local - WordPress website
...
4. (Recommended) Commit the 'export' commit to separate it from actual changes:
   ...
   ```
3. Run `scripts/start.sh` to launch docker environment
...
4. If you made changes in the WordPress admin panel, or the database directly, update the database dump:
   ```
...
6. Go to GitHub and create a Pull Request into `main` branch. Briefly describe the changes you have made, and mention relevant issue id. Add reviewers, or self-review. Once the PR is merge d, the changes are deployed automatically to https://harvest-staging-new.net in a few minutes. 
7. Make sure the changes are working on staging. 
8. Notify the QA person.
...
3. See the logs of individual services: `docker logs id_or_name_from_docker_ps`
4. Restart all services without losing data: `docker compose restart`
5. Stop all services without losing data: `docker compose stop`   
6. Shutdown all services: `docker compose down`. **WARNING: all database changes since the container start will be lost**
7. Clear all local changes: `git checkout --force .`
8. Access http://harvest.local/wp-admin as `superadmin/superadmin`
9. Access the database using `docker exec -it harvest-website-mariadb-1 mysql -uharvestj -pharvestj`
10. Connect the terminal to website container: `docker exec -it harvest-website-wordpress-1 bash`
...
7. Imitate Zoho - WordPress callback with doctor call results, substituting 'harvest_uuid' with 'uuid' meta field of the patient:
   ```
   curl -XPOST 'http://harvest.local/wp-json/harvest-api/update_membership?auth_token=R3Q4VXN5RzJVOnh3WXFZRVl6SThD' -H 'Content-Type: application/json' -d '{"harvest_uuid":"HARVEST_UUID","zoho_id":"51445000001507041","prescription_approved":"true","rejection_reason":"none"}'
   ```
